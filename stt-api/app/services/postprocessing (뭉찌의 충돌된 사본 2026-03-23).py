...
# 의료 문맥과 무관한 비의료 키워드 (환각 탐지용)
_NON_MEDICAL_INDICATORS: list[str] = [
    "동참", "참여해 주시기 바랍니다", "구독", "좋아요", "시청",
    "채널", "투표", "선거", "정치", "경제", "주식",
    "여행", "관광", "맛집", "레시피", "기업에 동참",
    "영상 제공", "자막 제공",
]
...
class TextPostProcessor:
    ...
    def process_segments(self, segments: list[dict]) -> list[dict]:
        ...
        for seg in segments:
            ...
            if confidence < 0.3 and self._is_non_medical_text(text):
                logger.debug("비의료 환각 제거 (confidence=%.2f): %s", confidence, text[:40])
                continue
    ...
    def _is_non_medical_text(self, text: str) -> bool:
        """텍스트가 의료 문맥과 무관한지 판별."""
        count = sum(1 for kw in _NON_MEDICAL_INDICATORS if kw in text)
        return count >= 1
    ...
    def is_valid_segment(self, text: str) -> bool:
        ...
        # 비한국어 환각 필터: 한국어(가-힣)가 전체의 30% 미만이면 무효
        # (CJK 한자, 영어만 나오는 환각 차단 — "暑 reading" 등)
        korean_chars = len(re.findall(r"[가-힣]", cleaned))
        total_alpha = len(re.findall(r"[가-힣a-zA-Z\u4e00-\u9fff]", cleaned))
        if total_alpha > 0 and korean_chars / total_alpha < 0.3:
            logger.debug("비한국어 환각 제거: %s", cleaned[:40])
            return False
